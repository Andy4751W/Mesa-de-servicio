...
const SEGURIDAD_SESION_AVISO = 15;
const SEGURIDAD_SESION_VENCIMIENTO = SEGURIDAD_SESION_INACTIVIDAD + SEGURIDAD_SESION_AVISO;
...
function seguridadAplicarCabeceras(bool $contenidoPrivado = true): void
{
    ...
    // El perfil se abre dentro de un iframe del mismo portal.
...
function seguridadIniciarSesion(): void
{
    ...
    ini_set('session.gc_maxlifetime', (string) SEGURIDAD_SESION_VENCIMIENTO);
...
function seguridadMotivoVencimiento(?int $ahora = null): ?string
{
    ...
    if ($ahora - $interaccion >= SEGURIDAD_SESION_VENCIMIENTO) {
        return 'inactividad';
    }
...
/** @return array{inactividad:int,aviso:int,gracia:int} */
function seguridadTiemposRestantes(?int $ahora = null): array
{
    $ahora ??= time();
    $interaccion = (int) ($_SESSION['ultima_interaccion'] ?? $ahora);

    $transcurrido = max(0, $ahora - $interaccion);

    return [
        'inactividad' => max(0, SEGURIDAD_SESION_INACTIVIDAD - $transcurrido),
        'aviso' => SEGURIDAD_SESION_AVISO,
        'gracia' => max(0, SEGURIDAD_SESION_VENCIMIENTO - $transcurrido),
    ];
}
...
function seguridadPasswordValida(string $password): bool
{
    ...
    $tieneMayuscula = preg_match('/\p{Lu}/u', $password) === 1;
    $tieneNumero = preg_match('/[0-9]/', $password) === 1;
