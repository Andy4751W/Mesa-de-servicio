    ...
    try {
        if ($accion === 'crear' || $accion === 'editar') {
            ...
            if ($accion === 'crear') {
                $stmt = $conn->prepare(
                    "INSERT INTO sla
                        (id_pais_operacion, nombre, tiempo_respuesta, unidad, estado)
                     VALUES (?, ?, ?, ?, 'activo')"
                );
                $stmt->bind_param('isis', $idPaisOperacion, $nombre, $tiempo, $unidad);
                $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare(
                "UPDATE sla
                 ...
                 WHERE id_sla = ? AND id_pais_operacion = ?"
            );
            $stmt->bind_param('sisii', $nombre, $tiempo, $unidad, $idSla, $idPaisOperacion);
            $stmt->execute();

            if ($stmt->affected_rows === 0) {
                $verificar = $conn->prepare(
                    "SELECT id_sla FROM sla
                     WHERE id_sla = ? AND id_pais_operacion = ?"
                );
                $verificar->bind_param('ii', $idSla, $idPaisOperacion);
                $verificar->execute();
        ...
        if ($accion === 'cambiar_estado') {
            ...
            $stmt = $conn->prepare(
                "UPDATE sla SET estado = ?
                 WHERE id_sla = ? AND id_pais_operacion = ?"
            );
            $stmt->bind_param('sii', $nuevoEstado, $idSla, $idPaisOperacion);
            $stmt->execute();
        ...
        if ($accion === 'eliminar') {
            ...
            $stmtUso->bind_param('ii', $idSla, $idPaisOperacion);
            $stmtUso->execute();
            ...
            $stmt = $conn->prepare(
                "DELETE FROM sla WHERE id_sla = ? AND id_pais_operacion = ?"
            );
...
$resultadoSla = $conn->query(
    "SELECT
     ...
     LEFT JOIN servicios AS s
        ON s.id_sla = sl.id_sla
       AND s.id_pais_operacion = sl.id_pais_operacion
     WHERE sl.id_pais_operacion = {$idPaisOperacion}
     GROUP BY
        sl.id_sla,
        sl.nombre,
        sl.tiempo_respuesta,
