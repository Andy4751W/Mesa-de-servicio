    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .case-modal {
            ...
            justify-content:center;padding:20px;background:rgba(8,27,45,.68);backdrop-filter:blur(3px);
        }
        .case-modal.open { display:flex; }
        .case-dialog {
            width:min(1180px,calc(100vw - 34px));height:min(820px,calc(100vh - 34px));
        ...
        .modal-content {
            position:relative;min-height:0;display:block;padding:10px;overflow:hidden;
        }
        .tree-panel,.detail-panel {
            min-height:0;overflow:auto;border:1px solid #dce6f0;border-radius:11px;background:#fff;
        }
        .tree-panel { height:100%; }
        .detail-panel {
            position:absolute;inset:10px auto 10px 10px;z-index:8;width:min(560px,calc(100% - 20px));
            visibility:hidden;transform:translateX(calc(-100% - 24px));opacity:0;
            box-shadow:16px 0 45px rgba(6,31,55,.24);transition:transform .2s ease,opacity .2s ease,visibility .2s;
        }
        .detail-panel.open { visibility:visible;transform:translateX(0);opacity:1; }
        ...
        @media (max-width:760px) {
            ...
            .detail-panel { inset:7px;width:calc(100% - 14px); }
        }
    </style>
</head>
<body>
...
(function () {
    ...
    function prepararPanelLateral(titulo, referencia, secciones) {
        ...
        cerrarPanel.setAttribute('aria-label', 'Cerrar información seleccionada');
    ...
    function renderizarHistorico(vista, nodo) {
        ...
        eventos.forEach(function (evento) {
            ...
            item.appendChild(crearElemento('strong', '', evento.accion || 'Actividad registrada'));
            item.appendChild(crearElemento('p', '', evento.detalle || 'Sin detalle'));
            item.appendChild(crearElemento('div', 'event-meta', (evento.usuario || 'Sistema') + ' · ' + formatearFecha(evento.creado_en)));
            stream.appendChild(item);
        });
        vista.appendChild(stream);
    }
    ...
    function renderizarDetalleCaso(caso) {
        ...
        prepararPanelLateral('Información del caso padre', 'Caso ' + caso.id, [
            {
                titulo: 'Información',
    ...
    function renderizarArbol(datos) {
        ...
        const panelDetalle = contenidoModal.querySelector('.detail-panel');
