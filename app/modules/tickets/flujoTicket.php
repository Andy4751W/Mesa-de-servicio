...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .case-chat-stage-option{display:grid;grid-template-columns:29px minmax(0,1fr) 22px;gap:8px;align-items:center;min-height:43px;padding:6px 7px;border-radius:9px;color:#31546f;text-decoration:none;transition:background .15s ease,color .15s ease}.case-chat-stage-option:hover{color:#0b4fae;background:#eef6ff}.case-chat-stage-option.is-active{color:#0b4fae;background:#e7f2ff}
            ...
            <div>
                        ...
                        <?php if ($esGestorNodo): ?>
                            <div class="email-preference">
                                <div class="email-preference-copy">
                                    ...
                                    <p>Están activadas por defecto mientras este caso continúe asignado a usted.</p>
                                    <span class="email-preference-status <?= $notificacionesEmailGestor ? '' : 'off' ?>">
                                        <?= $notificacionesEmailGestor ? 'Notificaciones activadas' : 'Notificaciones desactivadas' ?>
                                    </span>
                                </div>
                                <form method="post">
                                    <input type="hidden" name="csrf_token" value="<?= escaparFlujo($_SESSION['csrf_token']) ?>">
                                    ...
                                    <button class="btn email-toggle" type="submit">
                                        <?= $notificacionesEmailGestor ? 'No recibir notificaciones' : 'Recibir notificaciones' ?>
                                    </button>
                                </form>
                            </div>
                        <?php endif; ?>
                ...
                >
                    ...
                    >
                        ...
                        <span class="case-chat-launcher-copy">
                            <strong>Conversación</strong>
                            <small>Solicitante y gestores</small>
                        </span>
                    </button>
                    ...
                    >
                        <header class="case-chat-header">
                            ...
                            <div class="case-chat-header-copy">
                                <strong><?= escaparFlujo($etiquetaNodoActual) ?> · Conversación</strong>
                                <span><?= $conversacionActual
                                    ? escaparFlujo(
                                        $chatActualInterno
                                    ...
                                    : 'Sin conversación habilitada' ?></span>
                            </div>
                        ...
                        <?php if ($conversaciones): ?>
                            ...
                            <div class="case-chat-stage">
                                ...
                                <details class="case-chat-picker" data-case-chat-picker>
                                    <summary aria-label="Seleccionar otra conversación">
                                        <span class="case-chat-stage-number"><?= $indiceConversacionActiva + 1 ?></span>
                                        <span class="case-chat-stage-current">
                                            <strong><?= escaparFlujo($tituloConversacionActiva) ?></strong>
                                            <small><?= $mensajesConversacionActiva ?> <?= $mensajesConversacionActiva === 1 ? 'mensaje' : 'mensajes' ?> · <?= $adjuntosConversacionActiva ?> <?= $adjuntosConversacionActiva === 1 ? 'archivo' : 'archivos' ?></small>
                                        </span>
                        ...
                        <div class="case-chat-timeline" data-case-chat-timeline aria-live="polite">
                            ...
                            <?php foreach ($eventosConversacion as $eventoConversacion): ?>
                                ...
                                <?php if (!$esAdjuntoChat): ?>
                                    <article class="case-chat-bubble <?= $esEventoPropio ? 'mine' : '' ?>">
                                        <span class="case-chat-author"><?= $esEventoPropio ? 'Tú' : escaparFlujo($eventoConversacion['autor'] ?: 'Usuario') ?></span>
                                ...
                                <?php else: ?>
                                    ...
                                    ?>
                                    <article class="case-chat-bubble is-file <?= $esEventoPropio ? 'mine' : '' ?>">
                                        <span class="case-chat-author"><?= $esEventoPropio ? 'Tú' : escaparFlujo($eventoConversacion['autor'] ?: 'Usuario') ?></span>
...
<script>
    (() => {
        ...
        const chatLauncher = chatWidget?.querySelector('[data-case-chat-open]');
        ...
        const appendOptimisticCaseChat = (message, files) => {
            ...
            if (message) {
                ...
                article.className = 'case-chat-bubble mine is-pending';
                const author = document.createElement('span');
                author.className = 'case-chat-author';
                ...
                bubbles.push(article);
            }

            files.forEach((file) => {
                const article = document.createElement('article');
                article.className = 'case-chat-bubble is-file mine is-pending';
                const author = document.createElement('span');
                author.className = 'case-chat-author';
                author.textContent = 'Tú';
                ...
                type.textContent = 'Archivo adjunto';
                copy.append(name, type);
                card.append(icon, copy);
                article.append(author, card);
                appendTime(article);
                chatTimeline.appendChild(article);
                bubbles.push(article);
            });
        ...
        chatForm?.addEventListener('submit', async (event) => {
            ...
            const bubbles = appendOptimisticCaseChat(message, files);
            ...
            try {
                ...
                bubbles.forEach((bubble) => {
                    ...
                    const time = bubble.querySelector('.case-chat-time');
                    if (time) {
                        time.textContent = result.sent_at || 'Ahora';
                        const mark = document.createElement('span');
                        mark.className = 'read-mark';
                        mark.setAttribute('aria-label', 'Enviado');
                        mark.textContent = '✓';
