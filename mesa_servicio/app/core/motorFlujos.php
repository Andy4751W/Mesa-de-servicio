 ...
 *
 * Cada vínculo directo del árbol conserva una conversación privada entre el
 * gestor que abrió el caso y el gestor asignado. Al crear hijos se pausa el
 * SLA del padre, los hermanos trabajan en paralelo y el padre se reanuda
 * cuando todos finalizan.
 */
...
function flujoCopiarChecklistPlantilla(
    ...
    $stmt->bind_param('ii', $idTicketEtapa, $idProcesoEtapa);
    $stmt->execute();
    $stmt->close();
}
...
function flujoCodigoCaso(
    ...
    $codigo = (string) (int) $ruta[0]['id_ticket_etapa'];
...
function flujoConversacionesDisponibles(
    ...
    foreach ($filas as &$fila) {
        $fila['codigo_caso'] = flujoCodigoCaso(
            $conn,
            $idTicket,
...
function flujoCrearTicket(
    ...
    try {
        ...
        flujoNotificar(
            ...
            "El caso raíz {$idEtapaActual} del ticket #{$idTicket} está disponible para su gestión."
        );
...
function flujoCrearCasosHijos(
    ...
    if ($rol !== 2) {
        throw new RuntimeException(
            'Solo el gestor propietario del caso puede crear casos hijos.'
        );
    }
    ...
    try {
        ...
        $stmt->bind_param('ii', $idTicket, $idNodoPadre);
        $stmt->execute();
        ...
        if (
            ...
            throw new RuntimeException(
                'El caso padre no está disponible para este gestor.'
            );
        }
        ...
        foreach ($normalizadas as $derivacion) {
            ...
            $stmtDestino->bind_param('i', $idServicioDestino);
            $stmtDestino->execute();
        ...
        foreach ($destinos as $itemDestino) {
            ...
            flujoRegistrarHistorial(
                ...
                $idTicket,
                $idGestorCreador,
                'Caso abierto por derivación',
                'Se abrió el caso '
                    . $codigoHijo
                    . ' desde el caso padre inmediato '
                    . $codigoPadre
                    . '. Destino: '
                    . $catalogoNombre
                    . ' / '
            ...
            flujoNotificar(
                ...
                $idTicket,
                $idHijo,
                'Nuevo caso hijo asignado',
                "Se derivó a su área el caso {$codigoHijo} del ticket #{$idTicket}."
            );
            ...
            if (
                ...
                flujoNotificar(
                    ...
                    "Se creó el caso hijo {$codigoHijo} para {$catalogoNombre} / {$servicioNombre}."
                );
            }
    ...
    if ($rol === 2) {
        $stmt = $conn->prepare(
            "SELECT 1
             ...
             WHERE t.id_ticket = ?
               AND t.id_proceso IS NOT NULL
               AND (t.id_usuario = ? OR te.id_ticket_etapa IS NOT NULL)
             LIMIT 1"
        );
        $stmt->bind_param('iii', $idUsuario, $idTicket, $idUsuario);
    } else {
        return false;
    }
...
function flujoPuedeEscribirNodo(
    ...
    if ($rol !== 2) {
        /* Administración conserva auditoría de lectura, no interviene en las
           conversaciones privadas de los gestores. */
        return false;
    }
    ...
    if ($rol === 2) {
        $stmt = $conn->prepare(
            "SELECT te.*
             ...
             INNER JOIN tickets AS t ON t.id_ticket = te.id_ticket
             WHERE te.id_ticket = ?
               AND (te.id_gestor = ? OR t.id_usuario = ?)
             ORDER BY
                (te.id_gestor = ?) DESC,
...
function flujoNotificarConversacion(
    ...
    foreach ($destinatarios as $idDestinatario) {
        ...
        flujoNotificar(
            ...
            $idTicket,
            $idTicketEtapa,
            'Nuevo mensaje en el caso ' . $codigo,
            $incluyeAdjuntos
                ? "Recibió un mensaje o documento en el chat privado del caso {$codigo}."
                : "Recibió un mensaje en el chat privado del caso {$codigo}."
        );
    }
}
...
function flujoMarcarEtapaLista(
    ...
    try {
        ...
        flujoRegistrarHistorial(
            ...
            'El gestor asignado marcó el caso '
                . $codigoCaso
        ...
        flujoNotificar(
            ...
            $idTicket,
            $idTicketEtapa,
            'Caso listo para cerrar',
            "El gestor asignado marcó el caso {$codigoCaso} como listo. Revise la solución y cierre o reabra la derivación."
        );
...
function flujoReabrirDerivacion(
    ...
    if ($rol !== 2 || !flujoModuloAprobacionCasosInstalado($conn)) {
    ...
    try {
        ...
        flujoRegistrarHistorial(
            ...
            $idTicket,
            $idUsuario,
            'Derivación reabierta',
            'El creador reabrió el caso '
                . $codigoCaso
        ...
        flujoNotificar(
            ...
            $idTicketEtapa,
            'Caso reabierto',
            "El creador reabrió el caso {$codigoCaso}. Debe continuar la gestión y el SLA volvió a correr."
        );
    ...
    try {
        ...
        flujoRegistrarHistorial(
            ...
            'Se cerró el caso '
                ...
                . ((int) ($etapa['id_ticket_etapa_padre'] ?? 0) > 0
                    ? ', hijo de ' . flujoCodigoCaso(
                        $conn,
                        $idTicket,
                        (int) $etapa['id_ticket_etapa_padre']
                    )
                    : ', caso padre')
                . '. '
        ...
        if ($idGestorAtencion !== $idUsuario) {
            flujoNotificar(
                ...
                $idTicket,
                $idTicketEtapa,
                'Caso aprobado y cerrado',
                'El creador aprobó y calificó el caso '
                    . flujoCodigoCaso($conn, $idTicket, $idTicketEtapa)
...
function flujoCalificarCerrarTicket(
    ...
    try {
        ...
        $stmt->bind_param('ii', $idTicket, $idCreador);
        $stmt->execute();
    ...
    if ($rol === 1) {
        ...
        $stmt = $conn->prepare(
            "SELECT DISTINCT t.*, p.nombre AS proceso_nombre,
             ...
             LEFT JOIN usuarios AS u ON u.id_usuario = t.id_usuario
             WHERE t.id_usuario = ? OR te.id_gestor = ?
             ORDER BY t.actualizado_en DESC, t.id_ticket DESC"
        );
