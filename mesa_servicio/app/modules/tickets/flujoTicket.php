 ...
 */
function ordenarArbolCasos(array $nodos): array
{
    $porPadre = [];
    $ids = [];
    ...
    $recorrer = static function (
        ...
        foreach ($porPadre[$idPadre] ?? [] as $indice => $nodo) {
            ...
            $nodo['codigo_caso'] = $idPadre === 0
                ? (string) $id
                : $codigoPadre . '.' . ($indice + 1);
            $ordenados[] = $nodo;
            $recorrer($id, $nivel + 1, (string) $nodo['codigo_caso']);
        }
    };
    $recorrer(0, 0);
...
function renderizarRamaCasos(
    ...
    $codigoCaso = (string) ($nodo['codigo_caso'] ?? $idNodo);
    $estado = (string) ($nodo['estado'] ?? 'pendiente');
    $estadoClase = preg_replace('/[^a-z0-9_-]/', '', strtolower($estado))
        ?: 'pendiente';
    $esSeleccionado = $idSeleccionado === $idNodo;
    $ramaActiva = isset($rutaSeleccionada[$idNodo]);
    ...
    $contenido = static function () use (
        ...
        <div class="case-row-main">
            <div class="case-code-line">
                <span class="case-code">Caso <?= escaparFlujo($codigoCaso) ?></span>
        ...
        >
            <summary class="case-row">
                ...
                <span class="branch-action">
                    <span class="when-open">− Minimizar rama · oculta <?= $totalDescendientes ?> caso<?= $totalDescendientes === 1 ? '' : 's' ?></span>
                    <span class="when-closed">＋ Expandir rama · muestra <?= $totalDescendientes ?> caso<?= $totalDescendientes === 1 ? '' : 's' ?></span>
                </span>
            </summary>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear_ticket') {
            if ($rol !== 2) {
                throw new RuntimeException('Solo los gestores pueden crear tickets.');
            }
            ...
            if (
                ...
                || !in_array($urgencia, ['baja', 'moderada', 'alta', 'urgente'], true)
            ) {
                redirigirFlujo('datos_incompletos');
            }

            $idTicket = flujoCrearTicket(
                $conn,
                $idProceso,
                $idUsuario,
                $titulo,
            ...
            $etapaActual = flujoEtapaActual($conn, $idTicket);

            if ($etapaActual && isset($_FILES['adjuntos'])) {
            ...
            redirigirFlujo('ticket_creado', $idTicket);
        }
        ...
        if ($accion === 'enviar_mensaje') {
            ...
            if (
                ...
                throw new RuntimeException('La conversación no está habilitada para su etapa.');
            }

            $mensaje = trim((string) ($_POST['mensaje'] ?? ''));
            $erroresAdjuntos = isset($_FILES['adjuntos'])
                ? (array) ($_FILES['adjuntos']['error'] ?? [])
                : [];
            ...
            try {
                ...
                flujoRegistrarHistorial(
                    ...
                    'Se agregó un mensaje o archivo al chat privado del caso '
                        . flujoCodigoCaso($conn, $idTicket, $idTicketEtapa)
                        . '.',
                    $idTicketEtapa
                );
                $conn->commit();
...
$tickets = flujoTicketsUsuario($conn, $idUsuario, $rol);
$serviciosDerivacion = flujoServiciosDisponibles($conn);
$notificaciones = [];
$stmtNotificaciones = $conn->prepare(
    "SELECT id_notificacion, id_ticket, titulo, mensaje, leida, creada_en
     FROM notificaciones
     WHERE id_usuario = ?
...
if ($idTicketSeleccionado > 0) {
    ...
    foreach ($etapas as $etapaArbol) {
        ...
        if ($idPadreArbol === 0 || !isset($etapasPorId[$idPadreArbol])) {
            $casosRaizVisuales[] = $etapaArbol;
        }
    }
...
$mensajes = [
    'ticket_creado' => ['ok', 'Ticket creado y primera área notificada.'],
    'caso_hijo_creado' => ['ok', 'Caso hijo creado. El SLA del padre quedó pausado.'],
    'casos_hijos_creados' => ['ok', 'Derivaciones creadas. Los casos hijos ya trabajan en paralelo y el SLA del padre quedó pausado.'],
    ...
    'ticket_cerrado' => ['ok', 'Las áreas y sus tiempos de respuesta fueron calificados. El ticket quedó cerrado definitivamente.'],
...
if ($rol === 2) {
    if ($idTicketSeleccionado > 0) {
        $modoGestor = 'detalle';
    } elseif (!in_array($modoGestor, ['nuevo', 'mis_tickets'], true)) {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .derivation-row {
            display: grid;
            grid-template-columns: minmax(190px, .9fr) minmax(240px, 1.25fr) auto;
            gap: 7px;
            align-items: end;
            padding: 8px;
            border: 1px solid var(--border);
...
<main class="shell">
    <header class="topbar">
        ...
        <div class="actions">
            ...
            <?php if ($rol === 2 && $idTicketSeleccionado > 0): ?>
                <a class="btn light" href="flujoTicket.php?modo=mis_tickets">Mis tickets</a>
            <?php elseif ($rol === 1): ?>
    ...
    <?php if ($rol === 2): ?>
        <nav class="modebar" aria-label="Opciones de tickets">
            <a class="mode-link <?= $modoGestor === 'nuevo' ? 'active' : '' ?>" href="flujoTicket.php?modo=nuevo">＋ Crear ticket</a>
            <a class="mode-link <?= in_array($modoGestor, ['mis_tickets', 'detalle'], true) ? 'active' : '' ?>" href="flujoTicket.php?modo=mis_tickets">
                Mis tickets <span class="count"><?= count($tickets) ?></span>
            </a>
        </nav>
    <?php endif; ?>
                ...
                <section class="card form-card" id="ticketFormPanel" hidden>
                    <div class="card-head">
                        <div>
                            ...
                            <p class="muted">Se crearán todas las etapas principales en orden. Cada gestor podrá derivar casos hijos cuando su etapa esté activa.</p>
                        </div>
                    ...
                    <form method="post" enctype="multipart/form-data" id="ticketCreateForm">
                        ...
                        <div class="form-actions">
                            <a class="btn light" href="flujoTicket.php?modo=mis_tickets">Cancelar</a>
    ...
    <?php elseif (!$ticketSeleccionado): ?>
        <section class="card">
            <div class="card-head">
                <div>
                    <h2><?= $rol === 2 ? 'Mis tickets creados y asignados' : 'Tickets' ?></h2>
                    <p class="muted">Consulte el estado y abra un ticket para revisar su trazabilidad.</p>
                </div>
                <?php if ($rol === 2): ?>
                    <a class="btn primary" href="flujoTicket.php?modo=nuevo">＋ Crear ticket</a>
                <?php endif; ?>
            </div>

            <?php if (!$tickets): ?>
                <div class="empty">
                    <strong>No hay tickets disponibles.</strong>
                    <?php if ($rol === 2): ?><p>Cree su primer ticket para iniciar una solicitud.</p><?php endif; ?>
                </div>
            <?php else: ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($tickets as $ticket): ?>
                                <tr>
                                    <td><span class="badge">#<?= (int) $ticket['id_ticket'] ?></span></td>
                                    ...
                                    <td>
                                        ...
                                        ?>
                                        <a class="btn <?= $esCierrePendiente ? 'primary' : 'light' ?>" href="<?= escaparFlujo($urlFila) ?>">
    ...
    <?php else: ?>
        <section class="card ticket-summary">
            <div>
                <span class="badge">Ticket #<?= (int) $ticketSeleccionado['id_ticket'] ?></span>
                <h2><?= escaparFlujo($ticketSeleccionado['titulo']) ?></h2>
                <p class="muted">
                    <?= escaparFlujo($ticketSeleccionado['proceso_nombre']) ?> ·
                    Gestor creador: <?= escaparFlujo($ticketSeleccionado['creador_nombre'] ?? $ticketSeleccionado['solicitante_nombre']) ?>
                </p>
        ...
        <?php if (
            ...
            <section class="closure-banner" aria-label="Cierre definitivo pendiente">
                <div>
                    <strong>Ticket anterior: falta la encuesta única del servicio solicitado</strong>
                ...
                <button class="primary" type="button" data-open-final-rating>
                    Calificar servicio y cerrar ticket
                </button>
            </section>
            ...
            >
                <div class="final-rating-shell">
                    <header class="final-rating-head">
                        <div>
                            <h2>Cierre definitivo del ticket #<?= $idTicketSeleccionado ?></h2>
                            <p>Esta acción registra una sola encuesta del servicio solicitado y cierra el ticket anterior.</p>
                        </div>
                        <form method="dialog">
                    ...
                    <div class="final-rating-body">
                        ...
                        <?php else: ?>
                            <div class="final-rating-intro">
                                ...
                                Las evaluaciones internas de casos hijos se registran por separado cuando el creador de cada derivación cierra su propio caso.
                            </div>
                                            ...
                                            <div class="rating-case-head">
                                                <div>
                                                    <div class="rating-case-title">
                                                        <span class="case-code">Caso <?= escaparFlujo($etapa['codigo_caso'] ?? $idEtapaCalificar) ?></span>
        ...
        <section class="card">
            <div class="card-head">
                <div>
                    <h2>Árbol de casos y trazabilidad</h2>
                    <p class="muted">Minimice un caso padre para ocultar toda la rama que depende de él. El caso padre siempre permanecerá visible.</p>
                </div>
            ...
            <div class="case-tree" data-case-tree data-tree-key="ticket-<?= $idTicketSeleccionado ?>">
                <?php if (!$casosRaizVisuales): ?>
                    <p class="muted">Este ticket todavía no tiene casos disponibles.</p>
                <?php endif; ?>
                <?php foreach ($casosRaizVisuales as $casoRaiz): ?>
                    <?php renderizarRamaCasos(
                        $casoRaiz,
                        $hijosPorPadre,
                        $rutaBaseTickets,
                ...
                <header class="case-modal-header">
                    <div>
                        <div class="case-modal-title-line">
                            <span class="case-code">Caso <?= escaparFlujo($etapaActual['codigo_caso'] ?? $etapaActual['id_ticket_etapa']) ?></span>
                        ...
                        <p>Revise y gestione únicamente la información de este caso.</p>
                    </div>
                    <form method="dialog">
                        <button class="modal-close" type="submit" aria-label="Cerrar ventana">×</button>
                    </form>
        ...
        >
            <div class="card-head">
                <div>
                    <h2>Acciones del caso <?= escaparFlujo($etapaActual['codigo_caso'] ?? '') ?></h2>
                    <p class="muted">Solo se muestran las acciones registradas dentro del caso seleccionado.</p>
                </div>
                ...
                >
                    <div class="card-head">
                        <div>
                            ...
                            <p class="muted">Cada chat pertenece a un vínculo directo entre quien abrió el caso y el gestor asignado.</p>
                        </div>
                    </div>
                    <div class="conversation-list">
                        <?php foreach ($conversaciones as $conversacion): ?>
                            ...
                            <a class="conversation-tab <?= $esConversacionActual ? 'active' : '' ?>" href="<?= escaparFlujo($urlConversacion) ?>">
                                <strong>Caso <?= escaparFlujo($conversacion['codigo_caso']) ?></strong>
                                <span><?= escaparFlujo($rolConversacion) ?></span>
                                <span><?= escaparFlujo($conversacion['gestor_origen'] ?: 'Gestor de origen') ?> ↔ <?= escaparFlujo($conversacion['gestor_destino'] ?: 'Gestor asignado') ?></span>
                    ...
                    <?php if ($conversacionActual): ?>
                        <div class="notice-panel" style="margin-bottom:8px">
                            <strong>Chat privado del caso <?= escaparFlujo($conversacionActual['codigo_caso']) ?></strong>
                            <div class="meta">
                                Abierto por <?= escaparFlujo($conversacionActual['gestor_origen'] ?: 'Gestor de origen') ?> ·
                    ...
                    <?php elseif ($conversacionActual): ?>
                        <p class="muted"><?= $rol === 1
                            ? 'Administración puede auditar esta conversación, pero solo sus dos gestores participantes pueden responder.'
            ...
            <div>
                <?php if ($etapaActual): ?>
                    ...
                    >
                        <div class="card-head">
                            <div>
                                <h2>Caso seleccionado <?= escaparFlujo($etapaActual['codigo_caso'] ?? $etapaActual['id_ticket_etapa']) ?></h2>
                        ...
                        <?php if ($estadoNodo === 'listo_cierre'): ?>
                            ...
                            <div class="ready-approval">
                                <div class="ready-approval-head">
                                    <div>
                                        <strong>Listo · pendiente de decisión del creador</strong>
                                ...
                                <?php if ($puedeCerrarNodo): ?>
                                    <form method="post" class="case-rating-form" onsubmit="return confirm('¿Confirma la calificación y el cierre de este caso?')">
                                        ...
                                        <p class="muted">Ningún caso padre o hijo puede cerrarse sin esta calificación.</p>
                                        <div class="rating-fields">
                                            <div>
                                        ...
                                        <div class="approval-actions">
                                            <button class="primary" type="submit">Calificar y cerrar caso</button>
                                        </div>
                                    </form>
                                    <form method="post" class="reopen-form" onsubmit="return confirm('¿Reabrir este caso? El corte de Listo se anulará y el SLA incluirá todo el tiempo transcurrido.')">
                                        ...
                                        <div class="approval-actions"><button class="btn danger" type="submit">Reabrir derivación</button></div>
                                    </form>
                                <?php else: ?>
                        ...
                        <?php if ($estadoNodo !== 'listo_cierre' && $puedeCrearHijo): ?>
                            <details class="case-panel-toggle" style="margin:10px 0">
                                <summary>Derivar a varias áreas · crear casos hijos</summary>
                                <form class="derivation-form" method="post" data-max-derivaciones="20">
                                    ...
                                    <div class="derivation-list" data-derivation-list>
                                        <div class="derivation-row" data-derivation-row>
                                            <div>
                                                <label>Área y servicio destino</label>
                                                <select name="derivaciones[0][id_servicio_destino]" required>
                                                    <option value="">Seleccione</option>
                                                    <?php foreach ($serviciosDerivacion as $servicioDestino): ?>
                                                        <option value="<?= (int) $servicioDestino['id_servicio'] ?>">
                                                            <?= escaparFlujo(
                                                                $servicioDestino['catalogo_nombre'] . ' / '
                                                                . $servicioDestino['servicio_nombre'] . ' · Gestor: '
                                                                . $servicioDestino['gestor_nombre'] . ' · SLA: '
                                                                . $servicioDestino['sla_nombre']
                                                            ) ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                    ...
                                    <div class="derivation-footer">
                                        ...
                                        <button class="primary" type="submit">Crear casos hijos</button>
                                    </div>
                                    ...
                                    <template data-derivation-template>
                                        <div class="derivation-row" data-derivation-row>
                                            <div>
                                                <label>Área y servicio destino</label>
                                                <select name="derivaciones[__INDEX__][id_servicio_destino]" required>
                                                    <option value="">Seleccione</option>
                                                    <?php foreach ($serviciosDerivacion as $servicioDestino): ?>
                                                        <option value="<?= (int) $servicioDestino['id_servicio'] ?>">
                                                            <?= escaparFlujo(
                        ...
                        <?php elseif (
                            ...
                            <div class="alert info" style="margin:10px 0">
                                <?php if ($rol === 1): ?>
                                    La trazabilidad está disponible, pero el caso hijo debe crearlo el gestor asignado a esta etapa.
                                <?php else: ?>
                        ...
                        <?php if ($estadoNodo !== 'listo_cierre' && $puedeGestionar): ?>
                            <details class="case-panel-toggle" style="margin-top:10px">
                                ...
                                <div class="case-panel-toggle-body">
                                    <form method="post">
                                        ...
                                        <input type="hidden" name="id_ticket" value="<?= $idTicketSeleccionado ?>">
                                        <input type="hidden" name="id_ticket_etapa" value="<?= (int) $etapaActual['id_ticket_etapa'] ?>">
                                        <?php if (!$checklist): ?><p class="muted">Esta etapa no tiene elementos configurados.</p><?php endif; ?>
                                        <?php foreach ($checklist as $item): ?>
                                            <div class="check">
                                                <label>
                                                    <input type="checkbox" name="completado[<?= (int) $item['id_ticket_checklist'] ?>]" value="1" <?= (int) $item['completado'] === 1 ? 'checked' : '' ?>>
                                                    <span>
                            ...
                            <details class="case-panel-toggle" style="margin-top:10px">
                                ...
                                <div class="case-panel-toggle-body">
                                    <form method="post" onsubmit="return confirm('¿Confirma que la solución está lista para revisión? El creador del caso será notificado para calificar, cerrar o reabrir.')">
                                        <input type="hidden" name="csrf_token" value="<?= escaparFlujo($_SESSION['csrf_token']) ?>">
                                        ...
                                        <?php if ($solucionesCaso): ?>
                                            ...
                                            <div class="form-actions"><button class="primary" type="submit">Marcar caso como listo</button></div>
                                        <?php else: ?>
    ...
    (() => {
        document.querySelectorAll('.derivation-form').forEach((form) => {
            ...
            form.addEventListener('submit', (event) => {
                ...
                if (!window.confirm(`¿Crear ${selected.length} caso(s) hijo(s) y pausar el SLA del caso actual?`)) {
                    event.preventDefault();
                }
            });
