...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($accion === 'cambiar_estado') {
            ...
            $stmt = $conn->prepare(
                'UPDATE feriados SET estado = ? WHERE id_feriado = ?'
            );
            $stmt->bind_param('si', $estado, $idFeriado);
            $stmt->execute();
            $stmt->close();
            redirigirFeriados(
                $estado === 'activo'
        ...
        if ($accion === 'eliminar') {
            ...
            $stmt = $conn->prepare(
                'DELETE FROM feriados WHERE id_feriado = ?'
            );
            $stmt->bind_param('i', $idFeriado);
            $stmt->execute();
            $stmt->close();
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .hero p { max-width: 700px; margin: 7px 0 0; color: rgba(255,255,255,.84); font-size: 12px; }
        .schedule-chip {
            display: inline-flex;
            margin-top: 13px;
            padding: 6px 10px;
...
<main class="page-shell">
    ...
    <section class="hero">
        ...
        <p>
            El SLA excluye automáticamente los sábados y domingos. Registre
            aquí cada festivo o rango horario que tampoco deba consumir tiempo
            de respuesta; únicamente los registros activos afectarán el cálculo.
        </p>
        <span class="schedule-chip">Horario base: lunes a viernes · 08:00 a 18:00 · Bogotá · festivos administrados manualmente</span>
    </section>
