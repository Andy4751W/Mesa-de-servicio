...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($accion === 'crear') {
            ...
            $stmt = $conn->prepare(
                "INSERT INTO catalogos (nombre, descripcion, imagen, estado)
                 VALUES (?, ?, ?, ?)"
            );
        ...
        if ($accion === 'editar') {
            ...
            $stmtImagen = $conn->prepare(
                "SELECT imagen FROM catalogos WHERE id_catalogo = ?"
            );
            $stmtImagen->bind_param("i", $idCatalogo);
            $stmtImagen->execute();
            ...
            $stmt->bind_param("sssi", $nombre, $descripcion, $imagen, $idCatalogo);
        ...
        if ($accion === 'cambiar_estado') {
            ...
            $stmt = $conn->prepare(
                "UPDATE catalogos SET estado = ? WHERE id_catalogo = ?"
            );
            $stmt->bind_param("si", $nuevoEstado, $idCatalogo);
        ...
        if ($accion === 'eliminar') {
            ...
            try {
                ...
                $stmtServicios = $conn->prepare(
                    "DELETE FROM servicios WHERE id_catalogo = ?"
                );
                $stmtServicios->bind_param("i", $idCatalogo);
    ...
    <div class="contenedor">
        ...
        <section class="hero">
            <div class="hero-copy">
                ...
                <p>
                    Cree y mantenga organizadas las clases de servicio que estarán
                    disponibles para los usuarios de la mesa de ayuda.
                </p>
            </div>
        </section>

        <section id="crear" class="panel">
            <div class="panel-encabezado">
                <div>
                    <h2>Crear nuevo catálogo</h2>
                    <p>Registre una nueva clase y el icono que la identificará.</p>
                </div>
                <span class="panel-etiqueta">Nuevo registro</span>
            </div>
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        <div>
                            ...
                            <input id="nuevo-nombre" type="text" name="nombre" maxlength="150" required>
                        </div>
                        ...
                        <div class="campo-completo">
                            ...
                            <textarea id="nueva-descripcion" name="descripcion"></textarea>
                        </div>
