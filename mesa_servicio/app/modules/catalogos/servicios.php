...
function opcionServicioDisponible($conn, $idOpcion, $tipo)
{
    ...
    $stmt->bind_param("is", $idOpcion, $tipo);
    $stmt->execute();
    $stmt->store_result();
    $disponible = $stmt->num_rows > 0;
    $stmt->close();
...
function relacionOpcionesServicioValida(
    ...
    $stmt->bind_param("isi", $idHijo, $tipoHijo, $idPadre);
    $stmt->execute();
    $stmt->store_result();
...
function leerOpcionesServicio(
    ...
    $relaciones = [
        ['id_ciudad', 'ciudad', 'id_pais'],
        ['id_lugar', 'lugar', 'id_ciudad'],
        ['id_departamento', 'departamento', 'id_lugar'],
    ];
    ...
    try {
        if (isset($_POST['nuevo_servicio'])) {
            ...
            $stmtCatalogo = $conn->prepare(
                "SELECT id_catalogo
                 FROM catalogos
                 WHERE id_catalogo = ? AND estado = 'activo'"
            );
            $stmtCatalogo->bind_param("i", $idCatalogo);
            ...
            $stmt = $conn->prepare(
                "UPDATE servicios
                 SET
                    ...
                    id_impacto = ?,
                    id_estado = ?
                 WHERE id_servicio = ? AND id_catalogo = ?"
            );
            $stmt->bind_param(
                "ssiiiiiiiiiiii",
                $nombre,
        ...
        if (isset($_POST['cambiar_estado_servicio'])) {
            ...
            $stmt->bind_param("sii", $nuevoEstado, $idServicio, $idCatalogo);
        ...
        if (isset($_POST['eliminar_servicio'])) {
            ...
            $stmt->bind_param("ii", $idServicio, $idCatalogo);
...
if ($idCatalogoSeleccionado) {
    ...
    $stmt->bind_param("i", $idCatalogoSeleccionado);
    $stmt->execute();
    ...
    if ($catalogoSeleccionado) {
        ...
        $stmtServicios->bind_param("i", $idCatalogoSeleccionado);
                            ...
                            >
                                ...
                                <div class="modal-form-grid">
                                    ...
                                    <div class="form-span-full">
                                        ...
                                        <textarea id="descripcion-servicio" name="descripcion"></textarea>
                                    </div>
                                    ...
                                    <p class="config-helper">
                                        Estos campos son opcionales. Las ciudades, lugares y
                                        departamentos se filtran según la selección anterior.
                                    </p>
                                    ...
                                    >
                                        ...
                                        <div class="form-edicion">
                                            ...
                                            <p class="config-helper">
                                                ...
                                                país, ciudad, lugar y departamento.
                                            </p>
        ...
        function inicializarOpcionesDeServicio() {
            ...
            document.querySelectorAll('.config-service-form').forEach(function (formulario) {
                ...
                function actualizarDesde(tipoPadre) {
                    ...
                    } else if (tipoPadre === 'ciudad') {
                        filtrarSelect('lugar');
                        filtrarSelect('departamento');
                    } else if (tipoPadre === 'lugar') {
                        filtrarSelect('departamento');
                    }
                }

                ['pais', 'ciudad', 'lugar'].forEach(function (tipoPadre) {
